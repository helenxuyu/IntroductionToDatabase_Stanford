...
where H1.ID = L1.ID1 and H2.ID = L1.ID2 and H1.ID not in (select L2.ID2 from Likes where H2.ID = L2.ID1);
...
where grade not in (select grade from Highschooler as H2, Friend where H2.ID = ID1 and H1.ID = ID2);

/*What is the average number of friends per student? (Your result should be just one number.)*/
select  avg(average)
from (select count(*) as average
from Highschooler, Friend
where ID =  ID1
group by ID) as nFriends;
...
from Friend as F1, Friend as F2, Highschooler as H
where F1.ID1 = H.ID and name = 'Cassandra'
and F2.ID1 = F1.ID2 and F2.ID2 <> H.ID;
...
group by ID
having count(ID2) = (select max(c) from (select ID1, count(*) as c from Friend group by ID1) as selected);
